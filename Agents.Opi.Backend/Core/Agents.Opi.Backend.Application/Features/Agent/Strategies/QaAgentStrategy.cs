...
public sealed class QaAgentStrategy : IAgentStrategy
{
    public AgentType AgentType => AgentType.QA;
    ...
    public string BuildPrompt(AgentPhase phase, string input, string? previousOutput)
    {
        ...
        return HekatePromptBuilder.Build(phase, input, previousOutput);
    }
    ...
    {
        AgentPhasePolicy.EnsureBelongsTo(AgentType, phase);
        return HekatePromptBuilder.BuildFeedback(phase, input, previousOutput, currentOutput, feedback);
    }

    public bool RequiresKnowledge(AgentPhase phase)
        => AgentPhasePolicy.RequiresIstqbKnowledge(AgentType, phase);
}
